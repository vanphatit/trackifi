...
export default function Cart() {
  ...
  const { subtotal, discount, total, freeShippingProgress } = useMemo(() => {
    if (!cart || !cart.items) return { subtotal: 0, discount: 0, total: 0, freeShippingProgress: { progress: 0, remaining: 0 } };

    const subtotal = cart.items.reduce((acc, item) => acc + item.product.price * item.quantity, 0);
    const discount = 0; // Placeholder
    const total = subtotal - discount; // Or use cart.total from API if it includes discounts
    
    const FREE_SHIPPING_THRESHOLD = 5000000;
    const progress = Math.min((total / FREE_SHIPPING_THRESHOLD) * 100, 100);
    const remaining = Math.max(FREE_SHIPPING_THRESHOLD - total, 0);
    
    return { subtotal, discount, total, freeShippingProgress: { progress, remaining } };
  }, [cart]);
  ...
  const handleRemove = (item: CartItem) => {
    if (window.confirm(`Bạn có chắc muốn xoá "${item.product.name}" khỏi giỏ hàng?`)) {
    ...
    return (
      <AppLayout contentClassName="mx-auto max-w-7xl px-4 py-8 lg:px-8">
        <div className="flex flex-col items-center justify-center rounded-3xl border border-slate-200 bg-white px-6 py-20 text-center shadow-sm">
          ...
          <h2 className="mt-8 text-3xl font-bold text-slate-900">Giỏ hàng trống trơn</h2>
          <p className="mt-3 max-w-md text-slate-500 text-lg">
            Có vẻ như bạn chưa chọn được món nào. Đừng bỏ lỡ các deal hời hôm nay nhé!
          </p>
          <Link
            to="/"
  ...
  return (
    <AppLayout contentClassName="mx-auto max-w-7xl px-4 py-8 lg:px-8">
      <div className="mb-8 flex items-center gap-3 text-sm text-slate-600">
        <Link to="/" className="inline-flex items-center gap-2 rounded-full border border-slate-200 px-3 py-1.5 text-slate-700 transition hover:border-red-300 hover:text-red-600">
      ...
      <div className="grid gap-8 lg:grid-cols-[1fr,380px] items-start">
        ...
        <div className="space-y-5">
          <div className="flex flex-wrap items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white px-6 py-5 shadow-sm">
            <h1 className="text-xl font-bold text-slate-900">Giỏ hàng ({cart.items.length} sản phẩm)</h1>
            <div className="flex items-center gap-2 text-sm font-medium text-emerald-600 bg-emerald-50 px-3 py-1 rounded-full">
              ...
              {freeShippingProgress.remaining > 0 
                ? `Mua thêm ${formatCurrency(freeShippingProgress.remaining)} để được Freeship`
          ...
          <div className="space-y-4">
            {cart.items.map((item) => (
              ...
              >
                {/* Image */}
                <div className="h-28 w-28 flex-shrink-0 overflow-hidden rounded-xl border border-slate-100 bg-slate-50 p-2">
                  {item.product.images && item.product.images.length > 0 ? (
                    <img src={item.product.images[0]} alt={item.product.name} className="h-full w-full object-contain mix-blend-multiply transition group-hover:scale-105" />
                  ) : (
                ...
                <div className="flex flex-1 flex-col justify-between gap-4 sm:flex-row sm:items-start">
                  <div className="space-y-2">
                    <h3 className="text-base font-bold text-slate-900 line-clamp-2">
                      <Link to={`/products/${item.product.id}`} className="transition hover:text-red-600">
                        {item.product.name}
                      </Link>
                    </h3>
                  ...
                  <div className="flex flex-col items-end gap-4 sm:gap-6">
                    ...
                    <div className="text-right">
                      ...
                      >
                        <Trash2 className="h-3 w-3 transition group-hover/del:scale-110" /> Xoá
                      </button>
                    </div>
                  </div>
                </div>
        ...
        <div className="space-y-4 lg:sticky lg:top-24">
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-lg shadow-slate-200/50">
            ...
            <div className="mt-6 space-y-3 border-b border-slate-100 pb-6">
              ...
              <div className="mt-2 flex justify-between items-center pt-2 border-t border-dashed border-slate-100">
                ...
                <div className="text-right">
                   <span className="block text-xl font-black text-red-600">{formatCurrency(total)}</span>
            ...
            <div className="mt-6">
              <label className="mb-2 block text-sm font-medium text-slate-700">Mã ưu đãi</label>
            ...
            <button 
              className="mt-6 w-full rounded-xl bg-red-600 py-4 text-sm font-bold text-white shadow-xl shadow-red-200 transition hover:bg-red-700 hover:shadow-red-300 hover:-translate-y-0.5"
              onClick={() => {
                if (isAuthenticated) {
                  navigate("/checkout");
                } else {
          ...
          <div className="rounded-2xl border border-slate-200 bg-blue-50 p-4 text-blue-800 shadow-sm">
             <div className="flex items-start gap-3">
               ...
               <div>
                 <p className="text-xs font-bold uppercase opacity-80 mb-1">Yên tâm mua sắm</p>

  ...
  useEffect(() => {
    ...
    async function fetchData() {
      ...
      try {
        ...
        if (mounted) setError(err.message || "Không thể tải sản phẩm. Vui lòng thử lại.");
      } finally {
          ...
          <div className="grid gap-8 p-6 lg:grid-cols-[1.1fr,1fr]">
            <div>
              <div className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
                {activeImage ? (
                  <img src={activeImage} alt={product.name} className="h-[420px] w-full object-contain" />
                ) : (
                  ...
                  {product.images.map((img) => (
                    <button
                      ...
                      className={`h-20 w-24 flex-shrink-0 overflow-hidden rounded-xl border transition ${
                        activeImage === img ? "border-red-400 ring-2 ring-red-100" : "border-slate-200"
                      }`}
                    >
            ...
            <div className="space-y-5">
              ...
              <p className="text-sm text-slate-600">{product.shortDescription || "Mô tả đang cập nhật."}</p>

              <div className="flex flex-wrap items-center gap-3">
                <div className="text-3xl font-bold text-red-600">{formatCurrency(finalPrice)}</div>
                {Number(product.discountPercent) > 0 ? (
                  <>
                    ...
                    <span className="inline-flex items-center gap-1 rounded-full bg-red-50 px-3 py-1 text-xs font-semibold text-red-600">
                      <BadgePercent className="h-4 w-4" /> -{product.discountPercent}%
                    </span>
                  </>
                ) : (
              ...
              <div className="flex flex-wrap items-center gap-3 text-sm text-slate-600">
                <div className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1">
                  ...
                  {reviews.length > 0 ? `${(reviews.reduce((acc, r) => acc + r.rating, 0) / reviews.length).toFixed(1)}/5` : "Chưa có đánh giá"} 
              ...
              <div className="flex flex-wrap items-center gap-3">
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 rounded-xl bg-red-500 px-5 py-3 text-sm font-semibold text-white shadow-lg transition hover:bg-red-600 focus:outline-none focus:ring-2 focus:ring-red-400 focus:ring-offset-2"
                >
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                ...
                <div className="mt-2 text-sm text-slate-600" dangerouslySetInnerHTML={{ __html: product.description || "Thông tin chi tiết đang được cập nhật." }} />
              </div>

              {specsEntries.length > 0 && (
                <div className="rounded-2xl border border-slate-200 bg-white p-4">
                  <p className="text-sm font-semibold text-slate-900">Thông số kỹ thuật</p>
                  <dl className="mt-3 grid gap-3 sm:grid-cols-2">
                    {specsEntries.map(([key, value]) => (
                      ...
                      >
                        ...
                        <dd className="pl-3 text-right text-slate-600">{value}</dd>
                      </div>
                    ))}
                  </dl>
                </div>
            ...
            {relatedProducts.map((p) => (
               <div
                ...
                >
                  ...
                  <Link to={`/products/${p.id}`} className="mt-4 text-sm font-semibold text-slate-900 line-clamp-2 min-h-[40px] hover:text-red-600 transition">
                    {p.name}
                  </Link>
